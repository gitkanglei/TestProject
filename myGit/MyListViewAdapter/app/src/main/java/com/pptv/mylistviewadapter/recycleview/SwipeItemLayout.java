...
public class SwipeItemLayout extends FrameLayout
{
    ...
    /**
     * 某些情况下，不能通过mIsOpen判断当前菜单是否开启或是关闭。 因为在调用 {@link #open()} 或者
     * {@link #close()} 的时候，mIsOpen的值已经被改变，但是
    ...
    /**
     * Menu的集合，以{@link android.view.Gravity#LEFT}和
     * {@link android.view.Gravity#LEFT}作为key， 菜单View作为value保存。
     */
    ...
    private boolean isOpenAnimating()
    {
        if (mCurrentMenu != null)
        {
            ...
            if (mIsOpen && ((isLeftMenu() && contentLeft < menuWidth) || (isRightMenu() && -contentLeft < menuWidth)))
            {
                return true;
            }
        }
